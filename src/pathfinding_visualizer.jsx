...
export default function PathfindingVisualizer() {
  ...
  // NEW state for auto-run-all feature
  const [autoRunningAlgo, setAutoRunningAlgo] = useState(null); // label of currently animating algo
  const [bestAlgo, setBestAlgo] = useState(null);               // { label, color, icon, pathLen, visited, time }
  const [allAlgoResults, setAllAlgoResults] = useState(null);   // all 4 results after run-all
  ...
  // Original single-algo visualize (▶ Visualize button — unchanged)
  function visualize() {
    if (isRunning) return;
    clearVisualization();
    setIsRunning(true);
  ...
  // NEW: Run all 4 algorithms sequentially with animation, then show the best path
  function runAllAndShowBest(currentGrid, currentStart, currentEnd) {
    ...
    // Pre-compute all steps synchronously
    ...
    // Best = shortest path; tie-break by fewest visited
    ...
    function runNextAlgo() {
      if (algoIdx >= allSteps.length) {
        // All done — show best path on grid
        setAutoRunningAlgo(null);
      ...
      // Instant mode — flash final state briefly then move on
      if (delay < 0) {
        if (last) {
          setVisitedCells(new Set(last.visited.map(([r,c]) => `${r},${c}`)));
          setFrontierCells(new Set());
      ...
      function tick() {
        if (i >= steps.length) {
          ...
          animRef.current = setTimeout(runNextAlgo, 400); // brief pause between algos
          return;
        }
        const s = steps[i];
        setVisitedCells(new Set(s.visited.map(([r,c]) => `${r},${c}`)));
    ...
    <div style={{ minHeight:"100vh", background:"#f8f9fc", fontFamily:"'Georgia',serif", color:"#1a1a2e", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ background:"#fff", borderBottom:"2px solid #e8ecf5", padding:"14px 28px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"12px", boxShadow:"0 2px 8px rgba(0,0,0,0.04)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"12px", flexWrap:"wrap" }}>
          {/* Auto-run in-progress indicator */}
          {autoRunningAlgo && (
              ...
              {ALGOS.map((a, i) => (
                <span key={a.id} style={{ display:"flex", alignItems:"center", gap:"4px" }}>
                  <span style={{
                    ...
                    color: a.label === autoRunningAlgo ? a.color : "#cbd5e1",
          ...
          {/* Best algo winner badge — shown after auto-run completes */}
          ...
          {/* Original stats badge — shown when using ▶ Visualize (single algo) */}
          {stats && !bestAlgo && (
      ...
      <div style={{ display:"flex", flex:"1", overflow:"hidden" }}>
        ...
        <div style={{ width:"260px", background:"#fff", borderRight:"1px solid #e8ecf5", padding:"20px 14px", display:"flex", flexDirection:"column", gap:"20px", overflowY:"auto" }}>
          ...
          <div>
            ...
            <div style={{fontSize:"10px",color:"#94a3b8",fontFamily:"'DM Mono',monospace",marginTop:"4px",lineHeight:"1.6"}}>
              Click grid to place <span style={{color:"#16a34a",fontWeight:"600"}}>Source</span> or <span style={{color:"#d97706",fontWeight:"600"}}>Target</span>, then press <span style={{color:"#059669",fontWeight:"600"}}>⚡ Run All & Compare</span>
            </div>
          </div>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          {/* Run All results panel — shown after ⚡ Run All & Compare completes */}
          {allAlgoResults && !autoRunningAlgo && (
            <div className="fade-up" style={{ background:"#fff", borderBottom:"1px solid #e8ecf5", padding:"16px 20px" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"12px" }}>
                ...
                <button onClick={() => setAllAlgoResults(null)} style={{ background:"none", border:"none", cursor:"pointer", color:"#94a3b8", fontSize:"16px" }}>✕</button>
              </div>
                ...
                {allAlgoResults.map(a => {
                  ...
                  return (
                    <div key={a.id} className="slide-in" style={{
                      ...
                      {isBest && (
                        <div style={{ position:"absolute", top:"-10px", left:"50%", transform:"translateX(-50%)", background:a.color, color:"#fff", fontSize:"9px", fontWeight:"700", letterSpacing:"1.5px", padding:"2px 8px", borderRadius:"4px", whiteSpace:"nowrap" }}>
                          🏆 BEST PATH
                        </div>
                      )}
              ...
              {!bestAlgo && (
                <div style={{ marginTop:"10px", fontSize:"11px", color:"#dc2626", fontFamily:"'DM Mono',monospace" }}>✗ No algorithm found a path. Try clearing walls or repositioning start/end.</div>
              )}
            </div>
          )}
