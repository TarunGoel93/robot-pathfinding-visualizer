...
export function RunAllResultsPanel({ allAlgoResults, bestAlgo, autoRunningAlgo, onClose }) {
  ...
  return (
    <div className="fade-up" style={{ background: "#fff", borderBottom: "1px solid #e8ecf5", padding: "12px 14px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
        <div style={{ fontSize: "10px", letterSpacing: "2px", color: "#94a3b8", fontFamily: "'DM Mono',monospace", textTransform: "uppercase" }}>
            ...
            <div key={a.id} className="slide-in" style={{
              ...
              {isBest && (
                <div style={{
                  ...
                  background: a.color, color: "#fff", fontSize: "8px", fontWeight: "700",
                  letterSpacing: "1.5px", padding: "2px 7px", borderRadius: "4px", whiteSpace: "nowrap"
                }}>
                  🏆 BEST PATH
                </div>
              )}
              <div style={{ display: "flex", alignItems: "center", gap: "5px", marginBottom: "6px" }}>
                ...
                <span style={{ fontWeight: "700", color: a.color, fontSize: "12px" }}>{a.label}</span>
              </div>
              <div style={{ fontSize: "10px", color: "#64748b", display: "flex", flexDirection: "column", gap: "2px", fontFamily: "'DM Mono',monospace" }}>
      ...
      {bestAlgo && (
        <div style={{ marginTop: "8px", fontSize: "10px", color: "#64748b", fontFamily: "'DM Mono',monospace" }}>
          ↳ Showing <span style={{ color: bestAlgo.color, fontWeight: "700" }}>{bestAlgo.icon} {bestAlgo.label}</span>'s path on the grid
        </div>
      )}
      {!bestAlgo && (
        <div style={{ marginTop: "8px", fontSize: "10px", color: "#dc2626", fontFamily: "'DM Mono',monospace" }}>
...
export function BenchmarkPanel({ benchmarks, ROWS, COLS, maxBenchTime, onClose }) {
  ...
  return (
    <div className="fade-up" style={{ background: "#fff", borderBottom: "1px solid #e8ecf5", padding: "12px 14px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
        <div style={{ fontSize: "10px", letterSpacing: "2px", color: "#94a3b8", fontFamily: "'DM Mono',monospace", textTransform: "uppercase" }}>
          Benchmark — {ROWS}×{COLS} grid · 20 runs avg
        </div>
        <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#94a3b8", fontSize: "16px" }}>✕</button>
      </div>
        ...
        {benchmarks.map((b, i) => (
          <div key={b.id} className="slide-in" style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "5px", marginBottom: "6px" }}>
              ...
              <span style={{ fontWeight: "700", color: b.color, fontSize: "12px" }}>{b.label}</span>
              {i === 0 && (
                <span style={{ marginLeft: "auto", fontSize: "9px", background: "#22c55e", color: "#fff", borderRadius: "4px", padding: "1px 5px" }}>FASTEST</span>
              )}
            </div>
            <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "17px", fontWeight: "700", color: b.color, marginBottom: "4px" }}>
              {b.time < 1 ? `${(b.time * 1000).toFixed(1)}µs` : `${b.time.toFixed(3)}ms`}
            </div>
            <div style={{ background: "#e2e8f0", borderRadius: "4px", height: "4px", marginBottom: "6px" }}>
